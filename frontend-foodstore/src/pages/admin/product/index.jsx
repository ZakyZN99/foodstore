...
const AddProduct = () => {
    ...
    const handleAddProduct = async () => {
        ...
        try {
            ...
            //     tags: selectedTags.map(tag => tag.name  ), // Assuming selectedTags is an array of objects with a 'value' property
            ...
            const formData = new FormData(); // Create a new FormData object
                    ...
                    const tagIds = selectedTags.map(tag => tag.name);
    ...
    try {
        ...
        `http://localhost:3000/api/categories/${editProductId}`,
        {
            name: editProductNam,
            description: editProductDesc,
            price: editProductPrice,
            image_url: editProductImage,
            category: editProductCatg,
            tags: editProductTag
        },{
            headers: {
            Authorization: `Bearer ${token}`,
    ...
    setProducts((prevCategories) =>
        prevCategories.map((prod) =>
            prod._id === editProductId ? response.data : prod
    )
    );
    ...
    <div className="max-w-[1200px] border-1 p-3 mx-30 mt-10">
        ...
        <div className="border-1 p-2 flex gap-1">
                        ...
                        <table>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td className="pl-[20px] pb-2"><input className="text-black pl-2 rounded-sm w-[100%] h-7" type="text" value={productPrice} onChange={(e) => setProductPrice(e.target.value)} /></td>
                                </tr>
                                <tr>
            ...
            <div className="mt-5 text-center items-center">
                            <h1 className="text-lg pb-3">LIST PRODUCT</h1>
                                ...
                                <tbody>
                                    {currentItems.map((product, index) => (
                                        <tr key={product._id}>
                                            ...
                                            {/* <td className="border">{product.tags.name}</td> */}
                                            <td className="border"  style={{ textAlign: "center" }}><img src={getImageUrl ? getImageUrl(product.image_url) : Image} alt="Product" style={{ maxWidth: "100px", maxHeight: "100px", display: "block", margin: "auto" }} />
                                            </td>
                                            <td className="p-2">
